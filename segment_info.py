...
CLASSES = ['address', 'birthday', 'countryside', 'identity number', 'name']
DEVICE = 'cuda'

best_model = torch.load('./info_segmentation.pth')
...
for image, gt_mask in test_dataset_vis:
    ...
    print(np.max(pr_mask))
    # print(np.unique(pr_mask), pr_mask.shape, np.where(pr_mask > 0))
    # print(np.repeat(pr_mask[:, np.newaxis, :, :], 3, axis=1).shape)
